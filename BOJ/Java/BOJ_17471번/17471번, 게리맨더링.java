...
public class Main {
	...
	private static void Solution(int cnt) {
		if(cnt==n+1) {
			for(int i=1; i<=n; i++) { // groupA, B 나누기
				if(visit[i]) groupA.add(i);
				else groupB.add(i);
			}
			
			if(groupA.size()==0 || groupB.size()==0) return; // 각 그룹에 적어도 하나의 구역을 포함하도록
			
			if(BFS(groupA) && BFS(groupB)) { // groupA, groupB 둘 다 이어져있는지 확인
				// 인구 차이 최솟값 계산
				int sum=0;
