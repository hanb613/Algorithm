...
public class Main {
    ...
    public static void Solution(int idx) {
    	if(idx == k) {
        	...
        	for(int t=0; t<k; t++) {
        		int tc = numArr[t];
        		int nx=oper[tc].x-oper[tc].s;
        	    int nx2=oper[tc].x+oper[tc].s;
        	    int ny=oper[tc].y+oper[tc].s;
        	    int ny2=oper[tc].y-oper[tc].s;
        		Rotation(nx, nx2, ny, ny2, newArr);
        	}
	...
	public static void Rotation(int nx, int nx2, int ny, int ny2, int[][] arr) {
	    ...
	    list.clear();
	    Rotation(nx+1, nx2-1, ny-1, ny2+1, arr);
	}
